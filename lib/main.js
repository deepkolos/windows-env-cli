...
const ENV_REG_PATH = 'HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Control\\Session Manager\\Environment';
(async function main() {
    if (hasOption('--help'))
    ...
    if (isList || isListPath) {
        if (isListPath) {
            ...
            if (key)
                printItem(await getEnvItem(key));
            else
                console.log('参数错误');
        }
        return;
    ...
    if (isEdit || isAddPath || isUnshift || isRemove || isPush || isSet) {
        ...
        if (isSet)
            options.splice(1, 0, 'set');
        if (isPush)
            options.splice(1, 0, 'push');
        if (isRemove)
            options.splice(1, 0, 'remove');
        if (isUnshift)
            options.splice(1, 0, 'unshift');
        if (isAddPath)
            options = ['Path', 'push', ...otherOptions];
        const [key, operator, value, hasExpandString] = formatOptions(options);
        try {
            if (operator === 'set') {
                const type = hasExpandString ? 'REG_EXPAND_SZ' : 'REG_SZ';
                await exec(`reg add "${ENV_REG_PATH}" /v ${key} /t ${type} /d ${value}`);
            }
            if (operator === 'push' || operator === 'unshift') {
                ...
                await exec(`reg add "${ENV_REG_PATH}" /f /v ${key} /t ${item.type} /d "${item.values.join(';')}"`);
            }
            if (operator === 'remove') {
